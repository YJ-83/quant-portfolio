...
def render_settings():
    ...
    # 탭 생성
    tab1, tab2, tab3, tab4, tab5 = st.tabs(["📊 전략 설정", "💾 데이터 설정", "🔗 API 연결", "📱 텔레그램 알림", "ℹ️ 시스템 정보"])
...
def render_telegram_settings():
    ...
    try:
        from data.telegram_alert import (
            load_telegram_config, save_telegram_config,
            test_telegram_connection, TelegramAlert, AlertManager
        )
    ...
    st.markdown("""
    <div class='api-card' style='background: linear-gradient(135deg, #0088cc 0%, #00a6ed 100%);'>
        <div style='position: relative; z-index: 1;'>
            <div style='display: flex; align-items: center; gap: 0.75rem; margin-bottom: 0.75rem;'>
                <span style='font-size: 2rem;'>📱</span>
                <h3 style='margin: 0; color: white; font-weight: 700;'>텔레그램 알림 설정</h3>
            </div>
            <p style='margin: 0; color: rgba(255,255,255,0.9); font-size: 0.95rem;'>
                가격 알림, 매매 시그널, 리밸런싱 알림을 텔레그램으로 받으세요
            </p>
        </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    if not telegram_available:
        st.error("❌ 텔레그램 모듈을 찾을 수 없습니다. data/telegram_alert.py 파일을 확인해주세요.")
        return
    ...
    with col1:
        if config.get('bot_token'):
            st.success("✅ Bot Token: 설정됨")
        else:
            st.error("❌ Bot Token: 미설정")
    with col2:
        if config.get('chat_id'):
            st.success(f"✅ Chat ID: {config.get('chat_id')}")
        else:
            st.warning("⚠️ Chat ID: 미설정")
    with col3:
    ...
    # 설정 가이드
    with st.expander("📖 텔레그램 봇 설정 가이드", expanded=not config.get('bot_token')):
        st.markdown("""
        ### 1️⃣ 텔레그램 봇 생성
        1. 텔레그램에서 **@BotFather**를 검색하여 대화를 시작합니다
        2. `/newbot` 명령어를 입력합니다
        3. 봇 이름과 사용자명을 입력합니다 (예: `MyQuantBot`, `my_quant_bot`)
        4. BotFather가 제공하는 **API Token**을 복사합니다

        ### 2️⃣ Chat ID 확인
        1. 생성한 봇에게 아무 메시지나 보냅니다
        2. 브라우저에서 `https://api.telegram.org/bot<YOUR_TOKEN>/getUpdates` 접속
        3. 응답에서 `"chat":{"id": 123456789}` 형태의 Chat ID를 확인합니다

        ### 3️⃣ 그룹 채팅에서 사용하기
        - 봇을 그룹에 초대한 후, 그룹에서 메시지를 보내면 Chat ID가 음수로 표시됩니다
        - 예: `-1001234567890`
        """)
    ...
    with col1:
        ...
        <div class='settings-card'>
            <div style='display: flex; align-items: center; gap: 0.5rem; margin-bottom: 0.75rem;'>
                <span style='font-size: 1.25rem;'>🤖</span>
                <span style='font-weight: 600; color: #333;'>Bot Token</span>
            </div>
        </div>
        """, unsafe_allow_html=True)
    ...
    with col2:
        ...
        <div class='settings-card'>
            <div style='display: flex; align-items: center; gap: 0.5rem; margin-bottom: 0.75rem;'>
                <span style='font-size: 1.25rem;'>💬</span>
                <span style='font-weight: 600; color: #333;'>Chat ID</span>
            </div>
        </div>
        """, unsafe_allow_html=True)
    ...
    <div class='section-header'>
        <span style='font-size: 1.75rem;'>🔔</span>
        <h3 style='margin: 0; color: #333;'>알림 옵션</h3>
    </div>
    """, unsafe_allow_html=True)
    ...
    with col1:
        ...
        price_alert = st.checkbox(
            "💰 가격 알림",
            value=config.get('price_alert', True),
            help="목표가/손절가 도달 시 알림",
            key="telegram_price_alert"
        )

        signal_alert = st.checkbox(
            "📊 시그널 알림",
            value=config.get('signal_alert', True),
            help="RSI, MACD 등 기술적 시그널 알림",
            key="telegram_signal_alert"
        )

    with col2:
        rebalance_alert = st.checkbox(
            "🔄 리밸런싱 알림",
            value=config.get('rebalance_alert', True),
            help="리밸런싱 일정 알림",
            key="telegram_rebalance_alert"
        )

        daily_summary = st.checkbox(
            "📈 일일 요약",
            value=config.get('daily_summary', False),
            help="매일 포트폴리오 요약 발송",
            key="telegram_daily_summary"
        )

        market_alert = st.checkbox(
            "🌐 시장 알림",
            value=config.get('market_alert', False),
            help="급등락/시장 이상 알림",
            key="telegram_market_alert"
        )
    ...
    with col1:
        if st.button("🔌 연결 테스트", use_container_width=True, key="telegram_test_btn"):
            if bot_token and chat_id:
                with st.spinner("텔레그램 연결 테스트 중..."):
    ...
    with col2:
        if st.button("📤 테스트 메시지 전송", use_container_width=True, key="telegram_send_test"):
            if bot_token and chat_id:
                with st.spinner("테스트 메시지 전송 중..."):
                    try:
                        alert = TelegramAlert(bot_token, chat_id)
                        test_msg = "🧪 퀀트 포트폴리오 테스트 메시지\n\n✅ 텔레그램 알림이 정상적으로 설정되었습니다!"
                        success = alert.send_message(test_msg)
                        if success:
                            st.success("✅ 테스트 메시지가 전송되었습니다!")
                        else:
                            st.error("❌ 메시지 전송 실패")
                    except Exception as e:
                        st.error(f"❌ 오류: {e}")
    ...
    with col3:
        if st.button("💾 설정 저장", type="primary", use_container_width=True, key="telegram_save_btn"):
            new_config = {
                'bot_token': bot_token,
                'chat_id': chat_id,
                'enabled': enabled,
            ...
            success, message = save_telegram_config(new_config)
            if success:
                st.success(f"✅ {message}")
            else:
                st.error(f"❌ {message}")
    ...
    <div class='section-header'>
        <span style='font-size: 1.75rem;'>💰</span>
        <h3 style='margin: 0; color: #333;'>가격 알림 관리</h3>
    </div>
    """, unsafe_allow_html=True)
    ...
    with col4:
        st.markdown("<br>", unsafe_allow_html=True)
        if st.button("➕ 알림 추가", key="add_alert_btn"):
            if alert_stock and alert_price > 0:
                ...
                st.session_state.price_alerts.append({
                    'stock_code': alert_stock,
                    'alert_type': alert_type,
                    'price': alert_price,
                    'created': datetime.now().strftime("%Y-%m-%d %H:%M")
                })
                st.success(f"✅ {alert_stock} {alert_type} {alert_price:,}원 알림이 추가되었습니다.")
            else:
                st.error("❌ 종목코드와 가격을 입력해주세요.")

    # 등록된 알림 목록
    if 'price_alerts' in st.session_state and st.session_state.price_alerts:
        st.markdown("<br>", unsafe_allow_html=True)
        st.markdown("**📋 등록된 가격 알림**")

        alerts_df = pd.DataFrame(st.session_state.price_alerts)
        alerts_df.columns = ['종목코드', '알림유형', '가격', '등록일시']
        alerts_df['가격'] = alerts_df['가격'].apply(lambda x: f"{x:,}원")

        st.dataframe(alerts_df, use_container_width=True, hide_index=True)

        if st.button("🗑️ 모든 알림 삭제", key="clear_alerts_btn"):
            st.session_state.price_alerts = []
